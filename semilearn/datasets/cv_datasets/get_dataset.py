...
def get_cv_dataset(args, reg_alg, cls_alg, dataset_name, num_labels, data_dir="./data", include_lb_to_ulb=True):
    """
    ...
    eval_dset = ImageDataset(reg_alg, cls_alg, test_data, test_targets, transform_val, False, None)
    test_dset = None

    if reg_alg == "fullysupervised":
        lb_dset = ImageDataset(reg_alg, cls_alg, train_data, train_targets, transform_weak, False, transform_strong)
    ...
    lb_dset = ImageDataset(reg_alg, cls_alg, lb_data, lb_targets, transform_weak, False, transform_strong)
    ulb_dset = ImageDataset(reg_alg, cls_alg, ulb_data, ulb_targets, transform_weak, True, transform_strong)
