...
def get_audio_dataset(args, reg_alg, cls_alg, dataset_name, num_labels, data_dir="./data", include_lb_to_ulb=True):
    """
    ...
    eval_dset = AudioDataset(reg_alg, cls_alg, test_data, test_targets, transform_val, False, None, args.sample_rate)
    test_dset = None
    ...
    if dataset_name.lower() in ["bvcc"]:
        ...
        eval_dset = AudioDataset(reg_alg, cls_alg, dev_data, dev_targets, transform_val, False, None, args.sample_rate)
        test_dset = AudioDataset(reg_alg, cls_alg, test_data, test_targets, transform_val, False, None, args.sample_rate)

    if reg_alg == "fullysupervised":
        lb_dset = AudioDataset(reg_alg, cls_alg, train_data, train_targets, transform_weak, False, transform_strong, args.sample_rate)
    ...
    lb_dset = AudioDataset(reg_alg, cls_alg, lb_data, lb_targets, transform_weak, False, transform_strong, args.sample_rate)
    ulb_dset = AudioDataset(reg_alg, cls_alg, ulb_data, ulb_targets, transform_weak, True, transform_strong, args.sample_rate)

    if reg_alg == "supervised" and cls_alg in [None, "supervised"]:
        ulb_dset = None
