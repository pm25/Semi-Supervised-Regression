...
def get_nlp_dataset(args, reg_alg, cls_alg, dataset_name, num_labels=40, data_dir="./data", include_lb_to_ulb=True):
    """
    ...
    eval_dset = BasicDataset(reg_alg, cls_alg, test_texts, test_targets, False)
    test_dset = None
    ...
    if dataset_name.lower() in ["yelp_review", "amazon_review"]:
        ...
        eval_dset = BasicDataset(reg_alg, cls_alg, dev_texts, dev_targets, False)
        test_dset = BasicDataset(reg_alg, cls_alg, test_texts, test_targets, False)

    if reg_alg == "fullysupervised":
        lb_dset = BasicDataset(reg_alg, cls_alg, train_texts, train_targets, False)
    ...
    lb_dset = BasicDataset(reg_alg, cls_alg, lb_texts, lb_targets, False)
    ulb_dset = BasicDataset(reg_alg, cls_alg, ulb_texts, ulb_targets, True)
