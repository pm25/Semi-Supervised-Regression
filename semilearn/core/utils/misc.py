...
def over_write_args_from_file(args, yml):
    ...
    with open(yml, "r", encoding="utf-8") as f:
        dic = yaml.YAML(typ="rt").load(f.read())
        dic = {k: dic[k] if dic[k] != "None" else None for k in dic}
        for k in dic:
...
class TorchMinMaxScaler:
    """
    This scaler transforms tensor data to a specified range using Min-Max scaling.

    Args:
        output_range (Tuple[float, float]): The desired output range of the data.
            Default is (0, 1).
    """
    ...
    def __init__(self, output_range=(0, 1)):
        ...
        if self.output_range[0] >= self.output_range[1]:
            raise ValueError(f"Minimum of desired feature range must be smaller than maximum. Got {output_range}.")
        self._input_min = torch.tensor(0.0)
        self._input_scale = torch.tensor(1.0)
        self._output_min = torch.tensor(output_range[0])
        self._output_scale = torch.tensor(output_range[1] - output_range[0])
    ...
    def fit(self, tensor):
        ...
        self._input_min = tensor.min()
        self._input_scale = tensor.max() - tensor.min()

    def transform(self, tensor):
        if self._input_scale == 0:
            return tensor.sub_(self._input_min).add_(self._output_min)
        tensor.sub_(self._input_min).div_(self._input_scale)
        tensor.mul_(self._output_scale).add_(self._output_min)
        return tensor

    def inverse_transform(self, tensor):
        if self._input_scale == 0:
            return tensor.sub_(self._output_min).add_(self._input_min)
        tensor.sub_(self._output_min).div_(self._output_scale)
        tensor.mul_(self._input_scale).add_(self._input_min)
        return tensor
