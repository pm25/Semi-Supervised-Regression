...
def cls_consistency_loss(logits, targets, name="ce", mask=None):
    """
    consistency regularization loss in semi-supervised learning (classification).
...
class ClsConsistencyLoss(nn.Module):
    """
    Wrapper for consistency loss
    """

    def forward(self, logits, targets, name="ce", mask=None):
        return cls_consistency_loss(logits, targets, name, mask)
